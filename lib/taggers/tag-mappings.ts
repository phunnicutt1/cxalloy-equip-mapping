 ...
 */
export const TAG_MAPPINGS: Record<string, { tags: string[]; confidence: number; patterns: string[] }> = {
 ...
 */
export function getEquipmentTagPatterns(equipmentType: string): string[] {
  const mapping = EQUIPMENT_TYPE_MAPPINGS[equipmentType.toUpperCase()];
 ...
 */
export function getUnitTagPatterns(unit: string): string[] {
 ...
 */
export function getObjectTypeTagPatterns(objectType: string): string[] {
  const mapping = OBJECT_TYPE_MAPPINGS[objectType?.toUpperCase()];

...
/**
 * Trio format validation result
 */
export interface TrioValidationResult extends ValidationResult {
  metadata: {
    sectionCount: number;
    estimatedPointCount: number;
    hasValidSeparators: boolean;
...
/**
 * Connector data validation result
 */
export interface ConnectorValidationResult extends ValidationResult {
  metadata: {
    rowCount: number;
    columnCount: number;
...
export class ValidationUtils {
  private static readonly REQUIRED_TRIO_TAGS = ['dis', 'point'];
  private static readonly COMMON_TRIO_TAGS = [
    'dis', 'bacnetCur', 'bacnetDesc', 'kind', 'point', 'unit', 
    'writable', 'cmd', 'bacnetWrite', 'bacnetWriteLevel'
  ];
  private static readonly BACNET_OBJECT_TYPES = [
   ...
   */
  static validateTrioFormat(content: string): TrioValidationResult {
    const result: TrioValidationResult = {
      isValid: true,
      errors: [],
      warnings: [],
      suggestions: [],
   ...
   */
  private static validateTrioSection(section: string): ValidationResult {
   ...
   */
  private static validateBACnetReference(value: string): boolean {
   ...
   */
  private static detectCommonTrioIssues(content: string): string[] {
    const issues: string[] = [];
   ...
   */
  private static generateTrioSuggestions(result: TrioValidationResult): string[] {
   ...
   */
  static validateConnectorData(csvContent: string): ConnectorValidationResult {
    ...
    try {
      ...
      let validRowCount = 0;
      const columnStats: Record<string, { type: string; uniqueCount: number; nullCount: number }> = {};
      ...
      // Set metadata
      result.metadata.dataTypes = Object.fromEntries(
        Object.entries(columnStats).map(([key, stat]) => [key, stat.type])
      );
      
      result.metadata.uniqueValues = Object.fromEntries(
        Object.entries(columnStats).map(([key, stat]) => [key, stat.uniqueCount])
      );
   ...
   * Validate file size
   */
  static validateFileSize(content: string, maxSizeBytes: number = 10 * 1024 * 1024): ValidationResult {
    const result: ValidationResult = {
      isValid: true,
      errors: [],
      warnings: []
 ...
 */
export function validateTrioFormat(content: string): TrioValidationResult {
  return ValidationUtils.validateTrioFormat(content);
}

export function validateConnectorData(csvContent: string): ConnectorValidationResult {
...
export function validateFile(
  fileName: string, 
  content: string, 
  allowedExtensions: string[] = ['trio', 'csv']
): ValidationResult {

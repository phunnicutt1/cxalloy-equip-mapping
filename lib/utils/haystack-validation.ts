...
export function generateComplianceReport(results: TagValidationResult[]): {
  ...
  const averageScore = results.reduce((acc, r) => acc + (r.compliance?.score || 0), 0) / totalPoints;
...
export function generateBatchComplianceReport(tagSets: HaystackTagSet[]): HaystackComplianceReport {
  ...
  return {
    ...
    commonIssues: [
      ...report.issues.errors.map(err => ({ issue: err.message, count: err.count, severity: 'high' as const })),
      ...report.issues.warnings.map(warn => ({ issue: warn.message, count: warn.count, severity: 'medium' as const }))
    ],
